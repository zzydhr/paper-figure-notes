...
一篇 18 图论文实测：prep 约 40 秒、tasks 2 秒、render 20 秒，而第 3 步读图抽取
要几十分钟。**几乎全部时间都在第 3 步。**

主因是**输出量**：18 张图产出约 900 KB JSON，比文本输入（529 KB 任务文件）还多，
而生成比阅读慢一个数量级——时间基本花在逐字往外写。

| 手段 | 怎么用 | 效果 | 代价 |
|------|--------|------|------|
| 降档位 | `tasks --level light` | 输出降到约 15~20%，最有效 | 不抽 n、统计、分组、溯源 |
| 选图 | `tasks --figures main` | 按图数比例省 | 跳过 Extended Data |
| 提并行 | 一个子 agent 一张图 | 墙钟取决于最慢那张 | 规则被重复读，输入略增 |

三者可叠加。`--level standard` 是中间档：保留分组与实验设计，不要 n / 统计 / 溯源。

**并行的做法**：任务文件互不依赖，可全部并行；唯一约束是每个子 agent 只写自己那几个
`extracted/*.json`，全部完成后再由主 agent 统一跑 `merge` 和 `render`。

**怎么选档位**：要做可重复性核查或跨文献汇总就用 `full`——n、统计方法、逐字段溯源
缺一不可，降档会让这些字段全空。只是想判断「这篇值不值得精读」，`light` 足够。
拿不准就用 `full`：省下的时间远不如漏掉 `n` 的代价大。

**不要压缩图片分辨率。** 切图约 1400×1800，压小能省输入，但坐标轴刻度和流式象限
