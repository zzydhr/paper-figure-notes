...
#### 想更快的话，先问用户要不要用下面三个选项

第 4 步（读图抽取）是全流程唯一慢的环节。实测一篇 18 图论文：prep 约 40 秒、
tasks 2 秒、render 20 秒，而抽取要几十分钟。

原因是**输出量**：18 张图产出约 900 KB JSON，比文本输入还多，而生成比阅读慢一个
数量级——时间基本花在逐字往外写。据此有三个可选加速手段，**由用户选，不要替他默认**：

| 手段 | 怎么用 | 效果 | 代价 |
|------|--------|------|------|
| 降档位 | `tasks --level light` | 输出降到约 15~20%，最有效 | 不抽 n、统计、分组、溯源，只留「做了什么+结论」 |
| 选图 | `tasks --figures main` | 按图数比例省（这篇省 55%） | 跳过 Extended Data |
| 提并行 | 一个 subagent 一张图 | 墙钟时间取决于最慢那张 | 规则被重复读，输入略增（但输入便宜、输出贵，划算） |

三者可叠加。`--level standard` 是中间档：保留分组与实验设计，不要 n / 统计 / 溯源。

**该问清楚再定**：用户要做可重复性核查或跨文献汇总，就得用 `full`——n、统计方法、
逐字段溯源缺一不可，降档会让这些字段全空。只是想快速判断「这篇值不值得精读」，
`light` 足够。拿不准就按 `full` 做，别擅自降档：省下的时间远不如漏掉 n 的代价大。

图片分辨率**不要动**。切图约 1400×1800，压小能省输入，但坐标轴刻度和流式象限
